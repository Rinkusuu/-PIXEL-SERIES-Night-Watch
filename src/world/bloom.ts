...
/** The canal walls are lit at this share of the city's rate. */
...
export function lampSpots(
  ...
  // enough to the water to throw a real reflection down it — which is the whole
  // reason for narrowing the river into a channel between them. Same list
  // `drawQuay` cuts dark, so a lit one always lands in an opening.
  //
  // Lit far more sparingly than the city's. These walls carry a thousand
  // openings between them at arm's length, and at the city's own rate their
  // halos merge into one wall of light down each edge of the frame. Near
  // windows have to be few to be worth anything.
