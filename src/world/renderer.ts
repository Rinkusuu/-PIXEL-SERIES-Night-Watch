...
export function createWorldRenderer() {
  ...
  function frame(target: CanvasRenderingContext2D, input: FrameInput): void {
    ...
    // to a channel in the first place. The lamp pass runs later still, so the
    // windows lit in `lampSpots` land on top of the dark ones cut here.
    drawQuay(target, w, hz, { wall: ladder.rail, dark: VIGNETTE_INK });
